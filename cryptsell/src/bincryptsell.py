...
def exitPos(entryprice):
    ...
    while True:
        ...
        logger.info (f"In Sell Position => ema20 = {str(round(ema20,2))} closeprice= {str(round(closeprice,2))} diff= {str(round(diff,2))}")
        time.sleep(30)
        if (diff > stoploss):
...
logFilePath = "./logs"
logFileName = logFilePath + '/' + coin + 'sell.log'
    ...
    while True:
        ...
        if (histval < hist):
            ...
            try:
                ...
                orderLogger.warning(f"Sell order executed at closeprice: {str(closeprice)} at {str(now)}")
                exitPos(closeprice)
                #send notification
                #exit position
                break
